...
export default function useTaxiData(filters: any) {
  ...
  if (minFare) queryParams.append("minFare", minFare)
  if (maxFare) queryParams.append("maxFare", maxFare)
  if (minDistance) queryParams.append("minDistance", minDistance)
  if (maxDistance) queryParams.append("maxDistance", maxDistance)
  if (paymentType) queryParams.append("paymentType", paymentType)

  const { data, error } = useSWR(`/api/taxi-data?${queryParams.toString()}`, fetcher)
